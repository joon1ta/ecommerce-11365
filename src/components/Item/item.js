...
const Item = ({title, description, price, pictureUrl, productId }) => {
  return (
  
      <div style={{marginBottom: 35,display: 'inline-block' }} >
        <Link to={`/itemDetail/${productId}`}>
        <img style={{padding: 15,width: 300,height: 450}} src={pictureUrl} alt={productId}/>
        </Link>
        <h5 >{title}</h5>
        <p>Precio: ${price}</p>
